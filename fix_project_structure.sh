...
source venv/bin/activate || {
  echo "Virtual environment not found. Please run install_freqtrade.sh first."
  exit 1
}
...
if [ ! -f "user_data/strategies/freqai_strategy.py" ]; then
  echo "Strategy file not found. Creating a basic FreqAI strategy..."
  cat > user_data/strategies/freqai_strategy.py << 'EOL'
import logging
...
if [ ! -f "config/freqtrade_config.json" ]; then
  ...
  cat > config/freqtrade_config.json << 'EOL'
{
    "max_open_trades": 5,
    "stake_currency": "USDT",
...
if [ ! -f "scripts/run_freqai_strategy.py" ]; then
  echo "Run script not found. Creating a basic run script..."
  mkdir -p scripts
  cat > scripts/run_freqai_strategy.py << 'EOL'
#!/usr/bin/env python3
import sys
import argparse
import logging
...
def main():
    ...
    cmd = f"./scripts/run_freqai_strategy.py trade --config config/freqtrade_config.json --strategy FreqAIOptimizedStrategy --freqaimodel LightGBMRegressorMultiTarget"
    logger.info(f"Running FreqTrade with arguments: {cmd}")

    # Execute the freqtrade command
...
if __name__ == "__main__":
    main()
EOL
  chmod +x scripts/run_freqai_strategy.py
fi
